...
func TestStressless_RoundTripsToTheSameWords(t *testing.T) {
	...
	for _, w := range corpus.Words() {
		span, err := ParseText(w)
		if err != nil {
			continue
		}
		stressless, err := Stressless(span)
		if err != nil {
...
func TestStressless_HasNoDiacriticsAndAnAdjunctPerWord(t *testing.T) {
	...
	span, err := ParseText("amlalú amlala")
	if err != nil {
		t.Fatal(err)
	}
	got, err := Stressless(span)
	if err != nil {
		t.Fatal(err)
	}
