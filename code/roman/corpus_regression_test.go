...
// roman.Words in this file come from the community Discord archive (see
...
func roundTripsOne(t *testing.T, word string) bool {
	...
	out := roman.Formative(f)
	back, err := roman.ParseFormative(out)
	if err != nil {
...
func TestFormative_RejectsChain(t *testing.T) {
	if _, err := roman.ParseFormative("heltyurëi-annarëi"); err == nil {
		t.Error("roman.Formative accepted a hyphenated chain as one formative")
	}
}
...
func TestCorpus_GlottalHeadedCs(t *testing.T) {
	for _, w := range []string{
		...
		if _, err := roman.ParseFormative(w); err == nil {
			t.Errorf("roman.Formative(%q) succeeded; its Cs holds a glottal, which §3.5 forbids", w)
		}
	}
}
...
// shape. These parsed into a roman.Formative with Cr = "w", which is not a
...
func TestCorpus_RootCannotStartWithHWY(t *testing.T) {
	for _, w := range []string{"awaçmas", "awaçmasá", "awaçmüsúi", "awáçmaisa"} {
		if _, err := roman.ParseFormative(w); err == nil {
			t.Errorf("roman.Formative(%q) succeeded; a root may not begin with w-", w)
		}
	}
}
