// This file writes the three adjunct classes the parse arm could
// already read: §4.1.1 single-affix, §4.1.2 multiple-affix and §4.3
// modular. Each is the inverse of the parse function named in its doc
// comment, and each returns an error rather than a lossy word where the
// grammar gives it no way to write what the value holds.
//
// §4.8's parsing adjunct is not here: it declares stress rather than
// meaning, so it is phonology, and phonology.ParsingAdjunctFor writes
// it.
